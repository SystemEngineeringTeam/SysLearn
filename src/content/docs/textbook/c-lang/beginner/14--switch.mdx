...
<Aside type="tip">
 <details> switch文では、`case`に値を指定して該当する処理を実行します。 入力した`num`の値によって、対応する曜日を`printf`で表示し、`break`でswitch文を抜けましょう。 </details> 
 </Aside> 
<details> 
...
<Aside type="tip"> 
 <details>if文の条件式に`i % 2 != 0`とすると、偶数でない時に実行する</details> 
</Aside> 
<details> 
 <summary>模範解答</summary>
```c
