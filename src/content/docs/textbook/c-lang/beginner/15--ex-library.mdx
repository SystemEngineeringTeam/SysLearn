...
```
<Aside type="caution">
`M_PI`は、標準のc言語において、`<math.h>`に定義されてない。
VSCodeでは、`#define _USE_MATH_DEFINES`を記述することで、`M_PI`を使用できるようになる。
コンパイラや実装環境に定義されている場合に使用できるので、M_PIは非推奨である。
`M_PI`は、C++11以降の標準ライブラリでは非推奨となっているため、c言語でも使用しないことが推奨される。
</Aside>
