...
1. w64devkit のダウンロード  
   ...
   <Aside>
      展開先に強い制約や意図はありません。  
      もし、展開先がユーザープロファイル内の場合、展開に管理者権限は不要です。 したがって、管理者として実行することすら不要です。
   </Aside>
...
HomeBrew がインストールされていない場合は, [こちらの記事](https://zenn.dev/ackylarkham/articles/c93d5ed1689459) を参考にインストールしてください。
